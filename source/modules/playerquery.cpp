...
static Detouring::Hook g_RecvfromHook("recvfrom", nullptr);

static void BuildStaticInfo()
{
	if (!g_pGameDLL || !g_pEngineServer || !g_pFileSystem)
		return;
...
{
	if (!Util::server || !g_pEngineServer)
		return;
	...
	{
		if (!tags.empty()) tags += " ";
		tags += "gmws:" + std::to_string(gamemode.workshopid);
	}
	if (!gamemode.category.empty())
	{
		if (!tags.empty()) tags += " ";
		tags += "gmc:" + gamemode.category;
	}
	if (!loc.empty())
	{
		if (!tags.empty()) tags += " ";
	...
	g_InfoCachePacket.WriteByte(17); // proto version
...
static ssize_t recvfrom_detour(SOCKET s, void* buf, recvlen_t buflen, int32_t flags, sockaddr* from, socklen_t* fromlen)
{
	...
	if (len < 5 || !g_bInfoCacheEnabled) return len;
	...
	if (type != 'T') return len;

	// C'est une requête A2S_INFO — on répond avec le cache
	...
	// Retourner -1 pour que le moteur ignore ce paquet
	errno = EWOULDBLOCK;
	return -1;
}
...
void CPlayerQueryModule::Init(CreateInterfaceFn* appfn, CreateInterfaceFn* gamefn)
{
	...
	if (GetNetSocket)
	{
		// netsocket_t is { int port, bool listening, int hUDP, int hTCP }
	...
	g_RecvfromHook = Detouring::Hook("recvfrom", (void*)recvfrom_detour);
